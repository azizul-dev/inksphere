...
import { toggleUserBan, updateUserRole } from "@/lib/actions/user";
          ...
          <div key={user._id} className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-2">
                <span className={`inline-flex rounded-full px-3 py-1 text-xs font-medium ${roleStyle(user.role)}`}>
                  {user.role}
                </span>
              ...
              {user.banned ? (
                <button
                  disabled={loadingId === user._id}
                  onClick={() => handleToggleBan(user._id, true)}
                  className="w-full rounded-xl bg-emerald-500 py-3 text-sm font-medium text-white transition hover:bg-emerald-600 disabled:opacity-50"
                >
                  Unban User
                </button>
              ) : (
